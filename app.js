...
async function loadVacancyTabData() {
    ...
    // حساب الإحصائيات (لا تغيير هنا)
    let totalRequired = 0;
    const { data: contracts } = await supabaseClient.from('contracts').select('locations_and_guards').eq('status', 'active');
    if (contracts) { contracts.forEach(c => { if (c.locations_and_guards) { c.locations_and_guards.forEach(l => { if (l.shifts) { l.shifts.forEach(s => { totalRequired += parseInt(s.guards_count) || 0; }); } }); } }); }
    const { count: assignedEmployees } = await supabaseClient.from('users').select('*', { count: 'exact', head: true }).not('project', 'is', null).eq('employment_status', 'نشط');
    requiredEl.textContent = totalRequired;
    assignedEl.textContent = assignedEmployees || 0;
    const gap = totalRequired - (assignedEmployees || 0);
    gapEl.textContent = gap;
    if (gapEl.parentElement?.parentElement) gapEl.parentElement.parentElement.style.setProperty('--stat-color', gap > 0 ? '#ef4444' : '#16a34a');

    // جلب الشواغر مع الحقول الجديدة
    const { data: vacancies, error } = await supabaseClient.from('job_vacancies').select('*, contracts(company_name)').order('created_at', { ascending: false });
    
    if (error) { listContainer.innerHTML = '<p style="color:red;">خطأ في تحميل الشواغر.</p>'; return console.error(error); }
    if (vacancies.length === 0) { listContainer.innerHTML = '<p style="text-align: center;">لا توجد شواغر مضافة حالياً.</p>'; return; }

    listContainer.innerHTML = `<div class="table-container"><table><thead><tr><th>المسمى الوظيفي</th><th>المشروع</th><th>الموقع المحدد</th><th>تابع لعقد</th><th>الحالة</th><th>إجراءات</th></tr></thead><tbody id="vacancies-table-body"></tbody></table></div>`;
    
    const tableBody = document.getElementById('vacancies-table-body');
    vacancies.forEach(vacancy => {
        // --- الجزء الجديد: إضافة ملاحظة للشواغر المؤقتة ---
        let titleHtml = vacancy.title;
        if (vacancy.is_temporary && vacancy.leave_end_date) {
            const endDate = new Date(vacancy.leave_end_date).toLocaleDateString('ar-SA');
            titleHtml += `<br><small style="color: #f59e0b; font-weight: normal;">(تغطية مؤقتة حتى ${endDate})</small>`;
        }
        // --- نهاية الجزء الجديد ---
        ...
        tableBody.insertAdjacentHTML('beforeend', `
            <tr>
                ...
                <td>${vacancy.contracts ? vacancy.contracts.company_name : 'غير محدد'}</td>
                <td><span class="status ${vacancy.status === 'open' ? 'active' : 'inactive'}">${vacancy.status === 'open' ? 'مفتوح' : 'مغلق'}</span></td>
                <td><button class="btn-action edit-vacancy-btn" data-id="${vacancy.id}"><i class="ph-bold ph-pencil-simple"></i></button><button class="btn-action delete-vacancy-btn" data-id="${vacancy.id}"><i class="ph-bold ph-trash"></i></button></td>
            </tr>
        `);
    });
}
...
if (requestActionBtn) {
    ...
    try {
        if (action === 'reject') {
            ...
            switch (stage) {
                case 'ops_escalate':
                    if (!confirm('هل أنت متأكد؟ سيتم رفع الطلب للموارد البشرية.')) { btn.disabled = false; return; }
                ...
                case 'hr_final':
                    // ... (لا تغيير هنا في منطق الموارد البشرية) ...
                    break;

                case 'direct_permission':
                    if (!confirm('هل أنت متأكد من قبول طلب الاستئذان؟')) { btn.disabled = false; return; }
                    
                    const permUserId = btn.dataset.userId;
                    const { data: permUser, error: permUserError } = await supabaseClient.from('users').select('*, job_vacancies(*)').eq('id', permUserId).single();
                    if (permUserError || !permUser || !permUser.job_vacancies) throw new Error('لا يمكن العثور على بيانات الموظف أو شاغره.');
                    
                    // 1. تحديث حالة الطلب إلى "مقبول"
                    await supabaseClient.from('employee_requests').update({ status: 'مقبول' }).eq('id', requestId);

                    // 2. تجهيز وفتح نافذة إنشاء التغطية بالبيانات المعبأة مسبقاً
                    const modal = document.getElementById('create-coverage-modal');
                    ...
                    document.getElementById('coverage-new-region').value = permUser.region || '';
                    document.getElementById('coverage-new-city').value = permUser.city || '';
                    document.getElementById('coverage-new-start-time').value = new Date().toTimeString().slice(0, 5);
                    document.getElementById('coverage-new-end-time').value = shift.end_time;
                    document.getElementById('coverage-new-pay').value = ''; // يترك فارغاً
                    document.getElementById('coverage-new-reason').value = 'استئذان';
                    
                    // إعادة تعيين حقول التعديل والربط لضمان أنها عملية إنشاء جديدة
                    document.getElementById('coverage-edit-id').value = '';
                    document.getElementById('coverage-link-vacancy').value = '';
                    document.getElementById('coverage-link-vacancy').disabled = true;
        ...
        // إعادة تحميل الصفحات ذات الصلة لتحديث البيانات
        if (document.getElementById('page-ops-review-requests')?.checkVisibility()) loadOpsReviewRequestsPage();
        if (document.getElementById('page-permission-requests')?.checkVisibility()) loadPermissionRequests();
        if (document.getElementById('page-leave-requests')?.checkVisibility()) loadLeaveRequests();
        // ... باقي دوال التحديث
